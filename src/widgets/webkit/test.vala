...
public class ValaBrowser : Window {
	...
	private void create_widgets () {
		...
		this.back_button = new ToolButton.from_stock (Gtk.Stock.GO_BACK);
		this.forward_button = new ToolButton.from_stock (Gtk.Stock.GO_FORWARD);
		this.reload_button = new ToolButton.from_stock (Gtk.Stock.REFRESH);
	...
	private void connect_signals () {
		...
		this.web_view.load_finished.connect((x)=> {
				print ("FINISHED: "+x.uri+"\n" );
				loaded = false;
				});

		this.web_view.load_started.connect ((req) => {
			print ("STARTED: "+ req.uri+"\n");
			if (!loaded) {
				loaded = true;
				if (req.uri==null) {
				//loaded = true;
				...
				this.url_bar.text = req.uri;
				var code = """
				<pre>
				0x8048000  mov eax, 33
				0x8048003  push ebp
				0x8048008  inc ecx ;; pirindola
				<a href="/bin/ls/Symbols">Symbols</a>
				</pre>

				"""
				;
				code+=this.url_bar.text;
				if (req.uri == "/bin/ls/Disassemble")
					this.web_view.load_html_string (code, "/bin/ls/food");
				else this.web_view.load_html_string ("palla", this.url_bar.text);

				print ("fuck yeah "+this.url_bar.text+"\n");
			}
			update_buttons ();
		});
		...
		/*
		   this.web_view.load_started.connect ((x) => {
		   print ("LOADING %s\n", x.get_uri ());
		   });
		 */
	}
	...
	private void on_activate () {
		...
		this.web_view.load_html_string ("palla", "/bin/ls/Disassemble");
	}
